...
This section acts as a basic introduction to THREDDS client catalogs.

## Basic Client Catalog Structure

THREDDS client catalogs are XML files that collect, organize, and describe accessible datasets for client-side consumption.
...
| 2 | The `catalog` element is the root element of the XML file.<br> It must declare the THREDDS catalog namespace with the `xmlns` attribute exactly as shown.|           
| 3 | Declares a `service` element named `odap` that will serve data (`serviceType`) via the `OPeNDAP` protocol. |
| 4 |The `dataset` element named `SAGE III Ozone 2006-10-31` is declared. <br> It references the `odap service` on the prior line using the `serviceName` attribute, meaning that it will be served via the OPeNDAP protocol.<br> The `urlPath` and `ID` attributes are used to [construct a URL](#constructing-an-access-url) with which to access the dataset.|
...
1. Find the `service` referenced by the `serviceName` attribute in the `dataset` element (in this case, `odap`):
...
2. Append the `service` element's `base` path to the server root to construct the service base URL:
   ...
   * serviceBaseUrl = serverRoot + serviceBasePath = `https://hostname:port/thredds/dodsC/`
3. Find the `urlPath` referenced by the `dataset` element:
   ~~~xml
   <dataset name="SAGE III Ozone 2006-10-31" serviceName="odap" urlPath="sage/20061031.nc" ID="20061031.nc"/>
   ~~~
4. Append the `dataset` `urlPath` to the `service` element's `base` URL to get the entire dataset access URL:
   * serviceBaseUrl = `https://hostname:port/thredds/dodsC/`
   * datasetUrlPath = `sage/20061031.nc`
   * datasetAccessUrl = serviceBaseUrl + datasetUrlPath = `https://hostname:port/thredds/dodsC/sage/20061031.nc`
...
{%include ahead.html content="  
We will look at the location and contents of the [default client catalog](default_catalog.html) that comes with TDS in another section.
" %} 

## Next Step

Next, we'll examine how to make [multiple datasets](nested_datasets.html) available to the client.