...
#### `catalog.xml` 
* The main THREDDS catalog for the TDS.
* Acts as the main root catalog for TDS configuration.
* Acts as a client catalog for the TDS.


#### `enhancedCatalog.xml` 
* An example catalog included with the TDS.
* Referenced from the `catalog.xml` file.

#### `threddsConfig.xml` 
* A configuration file for allowing **non-default** services in the TDS, configuring caching, etc.
